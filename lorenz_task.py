...
parser.add_argument('--serieslen', type=int, default=20,
                    help='length Lorenz time series')


args = parser.parse_args()
print(args)
if args.ortho == 'random':
    namefile = 'lorenz_log_ResESN'
elif args.ortho == 'cycle':
    namefile = 'lorenz_log_ResESN_C'
elif args.ortho == 'identity':
    namefile = 'lorenz_log_ResESN_I'
...
if args.Euler:
    namefile = 'lorenz_log_EuESN'

if args.ES2N:
    namefile = 'lorenz_log_ES2N'
...
train_dataset = get_lorenz(N=5, F=8, lag=lag, washout=washout, serieslen=args.serieslen)
valid_dataset = get_lorenz(N=5, F=8, lag=lag, washout=washout, serieslen=args.serieslen)
test_dataset = get_lorenz(N=5, F=8, lag=lag, washout=washout, serieslen=args.serieslen)
# Each one of train, valid, test dataset has (washout + lag + 100*serieslen) time steps
# In train I'll wash out 200 time steps, so if serieslen=2 there are effectively 200 time steps for training
...
for guess in range(args.test_trials):
    ...
    def test_esn(dataset, classifier, scaler):
        ...
        dataset = dataset[:, :(100*args.serieslen + washout)].to(device)
    ...
    dataset = train_dataset[:, :(100*args.serieslen + washout)].to(device)
    activations = model(dataset)[0].cpu().numpy()
    activations = activations[:, washout:]
    activations = activations.reshape(-1, args.n_hid)

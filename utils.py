...
class uodeCell(nn.Module):
    def __init__(self, n_inp, n_hid, dt, rho, device='cpu'):
        super(uodeCell, self).__init__()
        self.dt = dt
        self.x2h = nn.Linear(n_inp, n_hid) # here there is the bias
        h2h = 2 * torch.rand(n_hid, n_hid) - 1        
        h2h = spectral_norm_scaling(h2h, rho)
        self.h2h = nn.Parameter(h2h, requires_grad=True)

    def forward(self, x, hy, hz, hr, hj):
        neuron_connect = torch.tanh(self.x2h(x) + torch.matmul(hy, self.h2h))
        #neuron_connect = torch.tanh(
        #    torch.matmul(x, self.x2h) + torch.matmul(hy, self.h2h))
        j_uode = (3 * hj * hr * hz - 15/8 * hr**3 + neuron_connect) / hz**2
        hj = hj + self.dt * j_uode # explicit
        hr = hr + self.dt * hj # implicit
        hz = hz + self.dt * hr # implicit
        hy = hy + self.dt * hz # implicit
        return hy, hz, hr, hj

class uodeRNN(nn.Module):
    """
    Batch-first (B, L, I)
    """
    def __init__(self, n_inp, n_hid, n_out, dt, rho, device='cpu',
                 ):
        super(uodeRNN, self).__init__()
        self.n_hid = n_hid
        self.cell = uodeCell(n_inp,n_hid,dt,rho, device=device)
        self.readout = nn.Linear(n_hid, n_out)
        self.device = device

    def create_alternating_z_matrix(self, b, h):
        # Create a tensor with alternating 1s and -1s
        alternating_values = torch.tensor([1, -1] * (h // 2))
        # Repeat the alternating values to match the desired number of rows
        matrix = alternating_values.repeat(b, 1)
        return matrix

    def forward(self, x):
        ## initialize hidden states
        hy = torch.zeros(x.size(0),self.n_hid).to(self.device)
        #hy = 2*torch.rand(x.size(0),self.n_hid).to(self.device)-1
        # initialise z values to 1 or -1 with equal probability
        hz = self.create_alternating_z_matrix(x.size(0),self.n_hid).to(self.device)
        #hz = torch.ones(x.size(0),self.n_hid).to(self.device)
        hr = torch.zeros(x.size(0),self.n_hid).to(self.device)
        #hr = 2*torch.rand(x.size(0),self.n_hid).to(self.device)-1
        hj = torch.zeros(x.size(0),self.n_hid).to(self.device)
        #hj = 2*torch.rand(x.size(0),self.n_hid).to(self.device)-1

        for t in range(x.size(1)):
            hy, hz, hr, hj = self.cell(x[:, t],hy,hz,hr,hj)
        output = self.readout(hy)

        return output
    

class uodeESN(nn.Module):
    """
    Batch-first (B, L, I)
    """
    def __init__(self, n_inp, n_hid, dt, rho, input_scaling, bias_scaling, device='cpu'):
        super().__init__()
        self.n_hid = n_hid
        self.device = device
        self.dt = dt
        ...
        x2h = (2 * torch.rand(n_inp, self.n_hid) - 1) * input_scaling
        # alternative init
        # x2h = torch.rand(n_inp, n_hid) * input_scaling
        self.x2h = nn.Parameter(x2h, requires_grad=False)
